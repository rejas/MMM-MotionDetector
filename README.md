...
The two wake confirmation options only matter for the `v4l2` backend, when turning the room light on or off is picked up as motion because the camera adjusts its exposure over several frames. The defaults are general starting points, not values tuned for a specific camera.
